...
public class BoardDay1 extends JPanel implements KeyListener, ActionListener {
    ...
    protected void loadBoard() {
        ...
        try {
            // 使用类加载器加载资源
            InputStream is = getClass().getClassLoader().getResourceAsStream("maze.txt");
            if (is == null) {
                throw new RuntimeException("Could not load maze.txt");
            }
            
            Scanner inputFile = new Scanner(new BufferedReader(new InputStreamReader(is)));
