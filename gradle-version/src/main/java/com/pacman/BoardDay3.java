...
public class BoardDay3 extends BoardDay2 {
    ...
    private Clip loadClip(String name) throws LineUnavailableException, IOException, UnsupportedAudioFileException {
        Clip clip = AudioSystem.getClip();
        InputStream is = getClass().getClassLoader().getResourceAsStream("sounds/" + name);
        if (is == null) {
            throw new RuntimeException("Could not load sound: " + name);
        }
        
        // 需要使用BufferedInputStream来支持mark/reset
    ...
    private void loadSounds() {
        try {
            beginningSound = loadClip("GAMEBEGINNING.wav");
            chompSound = loadClip("pacchomp.wav");
            fruitSound = loadClip("fruiteat.wav");
            ghostSound = loadClip("GHOSTEATEN.wav");
            deathSound = loadClip("killed.wav");
            powerSound = loadClip("extrapac.wav");
            intermSound = loadClip("interm.wav");
        } catch (Exception e) {
